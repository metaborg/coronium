...
open class EclipseCreateInstallation : JavaExec() {
    ...
    val iniFile: Provider<Path> =
        finalDestination.flatMap { finalDestination -> os.map { os -> finalDestination.resolve(os.installationIniRelativePath) } }
    ...
    init {
        ...
        destination.convention(buildDirName.map { buildDirectoryName ->
            project.buildDir.toPath().resolve(buildDirectoryName)
        })
    ...
    override fun exec() {
        ...
        destination.get().toFile()
            .deleteRecursively() // Delete and recreate destination to ensure a clean installation.
