...
class BundlePlugin : Plugin<Project> {
    ...
    private fun configureConfigurations(project: Project) {
        ...
        val bundleEmbedImplementation = project.configurations.create(bundleEmbedImplementation) {
            description =
                "Implementation dependencies to Java libraries embedded into the bundle, with private visibility"
            isCanBeConsumed = false
            isCanBeResolved = false
            isVisible = false
        ...
        project.tasks.named<Jar>("jar").configure {
            ...
            manifest {
                attributes(
                    ...
                    Pair(
                        "-fixupmessages",
                        "Classpath is empty, cannot be found on the classpath"
                    ) // Ignore some warnings caused by current setup.
                )
            }
        }
    }
            ...
            doFirst {
                ...
                val bundle = run {
                    ...
                    for (artifact in project.requireBundleReexport.resolvedConfiguration.resolvedArtifacts) {
                        ...
                        builder.requiredBundles.add(
                            BundleDependency(
                                module.name,
                                version,
                                DependencyResolution.Mandatory,
                                DependencyVisibility.Reexport
                            )
                        )
                    }
