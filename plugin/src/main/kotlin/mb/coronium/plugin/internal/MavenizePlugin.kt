...
open class MavenizeExtension(project: Project) {
    ...
    init {
        ...
        majorVersion.convention("2022-06")
        minorVersion.convention("R")
    }

    internal val version: Provider<String> =
        majorVersion.flatMap { majorVersion -> minorVersion.map { minorVersion -> "$majorVersion-$minorVersion" } }
    internal val groupId: Provider<String> = version.map { version -> "eclipse-$version" }
    internal val prefixUrl: Provider<String> =
        version.map { version -> "eclipse-committers/$version/eclipse-committers-$version" }
    internal val url: Provider<String> =
        mirrorUrl.flatMap { mirrorUrl -> prefixUrl.flatMap { prefixUrl -> os.flatMap { os -> arch.map { arch -> "$mirrorUrl/$prefixUrl-${os.eclipseDownloadUrlArchiveSuffix}${arch.appDownloadUrlArchiveSuffix}.${os.eclipseDownloadUrlArchiveExtension}" } } } }
