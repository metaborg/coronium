...
open class EclipseInstallationAddJvm : DefaultTask() {
    ...
    @get:OutputDirectory
    val copyDestDir: Provider<Path> =
        createTask.flatMap { it.flatMap { it.buildDirName.map { project.buildDir.resolve("$it-jvm").toPath() } } }

    @get:Internal
    val copyDestAppDir: Provider<Path> =
        copyDestDir.flatMap { destDir -> createTask.flatMap { it.flatMap { it.appDirName.map { destDir.resolve(it) } } } }
    ...
    @get:OutputDirectory
    val jvmDestDir: Provider<Path> =
        copyDestAppDir.flatMap { destAppDir -> jvmDestRelativePath.map { destAppDir.resolve(it) } }


    @get:OutputFile
    val iniFile: Provider<Path> =
        copyDestAppDir.flatMap { destAppDir -> os.map { destAppDir.resolve(it.installationIniRelativePath) } }
